...
class TestCLIRunnerDirect:
    ...
    def test_capture_resets_cached_rich_stdout_console(self, capsys):
        ...
        def fake_main():
            get_stdout_console().print("inside capture")

        with patch("tests.integration.helpers.cli_runner_direct.main", side_effect=fake_main):
            with capture_main_execution(["dblift", "info"]) as result:
                assert result.returncode == 0
                assert "inside capture" in result.stdout

        assert "inside capture" not in capsys.readouterr().out

        get_stdout_console().print("after capture")
        assert "after capture" in capsys.readouterr().out
