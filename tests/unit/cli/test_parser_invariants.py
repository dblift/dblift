...
def test_snapshot_table_option_is_available_where_snapshots_are_used(
    subcmd_argv: List[str],
):
    parser = create_parser()

    args = parser.parse_args([*subcmd_argv, "--snapshot-table", "custom_snapshots"])

    assert args.snapshot_table == "custom_snapshots"
