"""Coverage for ``dblift.cli.main._format_version`` — the ``--version`` renderer.
...
class TestFormatVersion:
    def test_oss_only_is_single_line(self):
        ...
        from dblift.cli.main import _format_version

        with patch("dblift.cli.main._resolve_core_version", return_value="2.3.0"):
...
class TestResolveCoreVersion:
    def test_prefers_bundled_init_over_shadowed_metadata(self, tmp_path, monkeypatch):
        ...
        from dblift.cli.main import _resolve_core_version

        (tmp_path / "__init__.py").write_text('__version__ = "9.9.9-bundled"\n', encoding="utf-8")
        monkeypatch.setattr("dblift.cli.main._project_root", tmp_path)
...
class TestResolveManifestVersion:
    ...
    def test_reads_version_from_manifest(self, tmp_path, monkeypatch):
        from dblift.cli.main import _resolve_manifest_version
...
class TestFormatVersionWithManifest:
    ...
    def test_manifest_headlines_over_disagreeing_metadata(self):
        ...
        with patch("dblift.cli.main._resolve_core_version", return_value="2.0.0"):
            with patch("dblift.cli.main._resolve_manifest_version", return_value="3.2.0"):
                with patch("importlib.metadata.version", _fake_version(installed)):
                    out = _format_version()
    ...
    def test_no_manifest_is_unaffected(self):
        ...
        installed = {"dblift-pro": "2.4.0"}
        with patch("dblift.cli.main._resolve_core_version", return_value="2.3.0"):
            with patch("dblift.cli.main._resolve_manifest_version", return_value=None):
                with patch("importlib.metadata.version", _fake_version(installed)):
                    out = _format_version()
