...
def test_from_sqlalchemy_config_overlay_preserves_engine_database_url(tmp_path):
    migrations = tmp_path / "migrations"
    migrations.mkdir()
    engine_db = tmp_path / "engine.db"
    wrong_db = tmp_path / "wrong.db"
    engine = create_engine(f"sqlite:///{engine_db}")
    overlay = DbliftConfig.from_dict(
        {
            "database": {"type": "sqlite", "url": f"sqlite:///{wrong_db}"},
            "migrations": {"directory": str(migrations)},
            "logging": {"level": "DEBUG"},
        }
    )

    client = DBLiftClient.from_sqlalchemy(engine, migrations_dir=migrations, config=overlay)

    try:
        assert client.config.database.url == engine.url.render_as_string(hide_password=False)
        assert str(wrong_db) not in client.config.database.url
        assert client.config.logging.level == "DEBUG"
    finally:
        client.close()


def test_from_sqlalchemy_config_overlay_preserves_explicit_schema(tmp_path):
    migrations = tmp_path / "migrations"
    migrations.mkdir()
    ...
    try:
        assert client.config.database.schema == "main"
    finally:
        client.close()
