...
class TestCreateSchemaIfNotExistsFallbacks:
    def test_minimal_create_user_succeeds_grants_minimal_privileges(self):
        ...
        create_users = [c for c in calls if "CREATE USER" in c]
        fallback_password_line = next(
            line for line in create_users[1].splitlines() if "IDENTIFIED BY" in line
        )
        fallback_password = fallback_password_line.split('"')[1]
        assert any("CREATE ANY TABLE, CREATE ANY VIEW" in c for c in calls)
        assert len(fallback_password) <= 30

    def test_create_user_uses_oracle_compatible_temporary_password(self):
        ops, qe, log = _make_ops()
        qe.execute_query.return_value = [{"user_count": 0}]
        ...
        calls = [c.args[1] for c in qe.execute_statement.call_args_list]
        create_user = next(c for c in calls if "CREATE USER" in c)
