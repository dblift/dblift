...
def _qx_with_rows(rows_by_keyword):
    ...
    def _execute_query(connection, query, params=None):
        for keyword, rows in rows_by_keyword.items():
            if keyword == "ALL_MVIEWS" and "SELECT mview_name FROM ALL_MVIEWS" in query:
                return rows
            if keyword == "ALL_TABLES" and "ALL_TABLES" in query:
                return rows
            if keyword not in {"ALL_MVIEWS", "ALL_TABLES"} and keyword in query:
                return rows
        return []
