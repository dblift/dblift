...
def _extracts_the_object(dialect: str) -> bool:
    ...
    return any(o.name.lower() == "victim" for o in objects)
...
class TestExtractionAgreesWithSplittingPerDialect(unittest.TestCase):
    ...
    inside it, for every dialect that ships a regex parser.
    """

    def _assert_agree(self, dialect: str):
        splitter_says_live = _splits_out_the_live_statement(dialect)
        extraction_found_it = _extracts_the_object(dialect)
        self.assertEqual(
            splitter_says_live,
            extraction_found_it,
            f"{dialect}: splitting says live={splitter_says_live}, "
            f"extraction found object={extraction_found_it}",
        )
...
class TestMySqlExtractionDoesNotNestBlockComments(unittest.TestCase):
    ...
    comment, and extraction must find its object — the issue's own
    example.
    """
...
class TestSqlServerExtractionNestsBlockComments(unittest.TestCase):
    ...
    the matching outer ``*/`` is one comment, so ``victim`` must not be
    reported as an object.
    """

    def test_nested_comment_hides_the_statement(self):
        objects = SqlServerRegexParser().extract_objects(NESTED_COMMENT_SQL)
        self.assertFalse(any(o.name.lower() == "victim" for o in objects))
...
class TestDb2ExtractionDoesNotNestBlockComments(unittest.TestCase):
    ...
    CHANGELOG.md); extraction must match, not assume nesting.
    """
