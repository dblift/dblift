...
class TestEnrichColumnsWithIdentity(unittest.TestCase):
    ...
    def test_generation_kind_always_is_carried_onto_the_column(self):
        """A column declared ``GENERATED ALWAYS AS IDENTITY`` rejects
        caller-supplied values on INSERT; ``GENERATED BY DEFAULT`` accepts
        them. ``SqlColumn.identity_generation`` is the field that records
        which one, and ``PostgresqlQuirks.render_identity_clause`` already
        reads it -- but this enricher builds a fixed three-key dict
        (seed_value / increment_value / last_value) from each identity row,
        so there is no key a vendor identity query could deliver a
        generation kind through. ``identity_generation`` therefore stays
        ``None`` on every introspected column of every dialect, and an
        ALWAYS column re-renders as BY DEFAULT: the migration silently
        relaxes the constraint, and neither diff nor round-trip validation
        reports anything because the emitted DDL is valid.

        The fix is the seam, not the capture: carry a fourth key through so
        a query that supplies one is no longer discarded.
        """
        si, _ = _make_si(has_connection=True)
        si.vendor_queries = MagicMock()
        si.vendor_queries.get_identity_columns_query.return_value = ("SELECT ...", [])
        ...
        # And the consequence the drop actually causes: the rendered clause.
        # Seed/increment were captured from the row (1 / 1), so they must
        # appear in the identity options rather than being dropped.
        self.assertEqual(
            PostgresqlQuirks().render_identity_clause(col),
            "GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1)",
        )

    def test_generation_kind_by_default_is_carried_onto_the_column(self):
        """The other value ``identity_generation`` accepts. It renders the
        same clause the dropped-value column renders today, so it cannot on
        its own distinguish "captured" from "discarded" -- assert the
        attribute, not just the DDL.
        """
        si, _ = _make_si(has_connection=True)
        si.vendor_queries = MagicMock()
        si.vendor_queries.get_identity_columns_query.return_value = ("SELECT ...", [])
        ...
        self.assertTrue(col.is_identity)
        self.assertEqual(col.identity_generation, "BY DEFAULT")
        self.assertEqual(
            PostgresqlQuirks().render_identity_clause(col),
    ...
    def test_row_without_a_generation_kind_leaves_it_none(self):
        ...
        default: substituting "BY DEFAULT" here would turn "we never asked"
        into a positive claim, which is how a wrong capability propagates
        silently.
        """
        si, _ = _make_si(has_connection=True)
        si.vendor_queries = MagicMock()
    ...
    def test_failed_decode_does_not_leave_a_generation_kind_behind(self):
        """``identity_generation`` is a field ``render_identity_clause``
        reads, so it belongs on the same side of the decode failure as the
        seed/increment reset above: a column whose identity metadata could
        not be decoded is reset to ``is_identity = False`` and falls out of
        identity handling entirely. Leaving a generation kind on it would
        contradict that -- ``to_dict`` serializes ``identity_generation``,
        so diff would see a non-identity column still carrying ALWAYS.
        """
        si, _ = _make_si(has_connection=True)
        si.vendor_queries = MagicMock()
        si.vendor_queries.get_identity_columns_query.return_value = ("SELECT ...", [])
