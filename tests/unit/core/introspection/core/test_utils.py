"""Comprehensive tests for row access utility helpers."""
...
def test_introspection_utils_re_exports_row_access_helpers():
    """Transition import path should expose the neutral helper implementations."""
    assert compatibility_utils.get_row_value is get_row_value
    assert compatibility_utils.parse_json_array is parse_json_array
    assert compatibility_utils.parse_pg_options is parse_pg_options
    assert compatibility_utils.strip_leading_comments is strip_leading_comments
    assert compatibility_utils.to_int is to_int
