...
class TestForeignServer:
    ...
    def test_generate_basic_create_statement_minimal(self):
        ...
        server = ForeignServer("minimal_server", "minimal_fdw")
        result = server._generate_basic_create_statement()
        assert "CREATE SERVER" in result
        assert "FOREIGN DATA WRAPPER" in result
        assert "minimal_server" in result
        assert "minimal_fdw" in result

    def test_generate_basic_create_statement_with_options(self):
        """Test basic create statement with options."""
        server = ForeignServer(
            "test_server", "test_fdw", options={"key1": "value1", "key2": "value2"}
        )
        result = server._generate_basic_create_statement()
        assert "CREATE SERVER" in result
