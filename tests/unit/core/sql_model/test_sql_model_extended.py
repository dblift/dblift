...
class TestProcedure(unittest.TestCase):
    ...
    def test_basic_create_statement_mysql_func_deterministic(self):
        fn = Procedure(
            "fn1", is_function=True, body="SELECT 1", dialect="mysql", volatility="IMMUTABLE"
        )
        stmt = fn._generate_basic_create_statement()
        self.assertIn("DETERMINISTIC", stmt)

    def test_basic_create_statement_mysql_func_not_deterministic_default(self):
        fn = Procedure("fn1", is_function=True, body="SELECT 1", dialect="mysql")
        stmt = fn._generate_basic_create_statement()
        self.assertIn("NOT DETERMINISTIC", stmt)

    def test_basic_create_statement_mysql_security_definer(self):
        fn = Procedure(
            "fn1", is_function=True, body="SELECT 1", dialect="mysql", security_definer=True
        )
        stmt = fn._generate_basic_create_statement()
        self.assertIn("SQL SECURITY DEFINER", stmt)

    def test_basic_create_statement_mysql_security_invoker(self):
        fn = Procedure(
            "fn1", is_function=True, body="SELECT 1", dialect="mysql", security_definer=False
        )
        stmt = fn._generate_basic_create_statement()
        self.assertIn("SQL SECURITY INVOKER", stmt)

    def test_basic_create_statement_mysql_data_access(self):
        fn = Procedure(
            "fn1", is_function=True, body="SELECT 1", dialect="mysql", data_access="reads sql data"
        )
        stmt = fn._generate_basic_create_statement()
        self.assertIn("READS SQL DATA", stmt)

    def test_basic_create_statement_mysql_comment(self):
        fn = Procedure("fn1", is_function=True, body="SELECT 1", dialect="mysql", comment="my func")
        stmt = fn._generate_basic_create_statement()
        self.assertIn("COMMENT", stmt)

    def test_basic_create_statement_mysql_body_begin(self):
        fn = Procedure("fn1", is_function=True, body="BEGIN SELECT 1; END", dialect="mysql")
        stmt = fn._generate_basic_create_statement()
        self.assertIn("BEGIN SELECT 1; END", stmt)

    def test_basic_create_statement_system_function_skipped(self):
        fn = Procedure("<", is_function=True)
        stmt = fn._generate_basic_create_statement()
        self.assertEqual(stmt, "")

    def test_basic_create_statement_return_type_int_skipped(self):
        fn = Procedure("fn1", is_function=True, return_type=42)  # int, not str
        stmt = fn._generate_basic_create_statement()
        self.assertEqual(stmt, "")

    def test_basic_create_postgresql_language(self):
        fn = Procedure(
            "fn1", is_function=True, body="BEGIN END", dialect="postgresql", language="plpgsql"
        )
        stmt = fn._generate_basic_create_statement()
        self.assertIn("LANGUAGE", stmt)
        self.assertIn("plpgsql", stmt)

    def test_basic_create_no_body_no_params(self):
        proc = Procedure("sp1")
        stmt = proc._generate_basic_create_statement()
        self.assertIn("sp1()", stmt)

    def test_basic_create_with_params(self):
        p = Parameter("p1", "INT")
