"""Procedure Comparator for Drift Detection.

This module provides the ProcedureComparator class which compares procedure objects
...
from core.comparison.diff_models import ProcedureDiff
...
class ProcedureComparator:
    """Compares procedure objects and generates diff results.

    This class provides methods to compare procedure objects from different sources
    ...
    def compare_procedures(
        ...
        if _quirks.proc_skip_empty_comparison and not actual.parameters:
        ...
        # Compare definition
        def _normalize_procedure_body(body: Optional[str], definition: Optional[str] = None) -> str:
        ...
        expected_def = _normalize_procedure_body(
            expected_body, getattr(expected, "definition", None)
        )
        actual_def = _normalize_procedure_body(actual_body, getattr(actual, "definition", None))
        if _quirks.proc_skip_empty_comparison and not expected_body:
        ...
        if expected_vol != actual_vol:
            ...
            logger.info(
                f"Procedure '{proc_name}': volatility changed from {expected_vol or 'DEFAULT'} to {actual_vol or 'DEFAULT'}"
            )
        ...
        if expected_sec != actual_sec:
            ...
            logger.info(
                f"Procedure '{proc_name}': SECURITY DEFINER changed from {expected_sec} to {actual_sec}"
            )
        ...
        if _quirks.proc_skip_empty_comparison:
            ...
            if expected_definer != actual_definer:
                ...
                logger.info(
                    f"Procedure '{proc_name}': definer changed from {expected_definer or 'DEFAULT'} to {actual_definer or 'DEFAULT'}"
                )
            ...
            if expected_comment != actual_comment:
                ...
                logger.info(
                    f"Procedure '{proc_name}': comment changed from {expected_comment or 'NONE'} to {actual_comment or 'NONE'}"
                )
            ...
            if expected_data_access != actual_data_access:
                ...
                logger.info(
                    f"Procedure '{proc_name}': data access changed from {expected_data_access or 'DEFAULT'} to {actual_data_access or 'DEFAULT'}"
                )
