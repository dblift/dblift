...
class Trigger(SqlObject):
    ...
    def _generate_basic_create_statement(self) -> str:
        ...
        """
        if self.definition and self.definition.strip().upper().startswith("CREATE"):
        ...
        definer_clause = ""
        if quirks.trigger_supports_definer_clause and self.definer:
            definer_clause = f"DEFINER = {self._format_mysql_definer(self.definer)} "
        ...
        if self.orientation == "ROW" and quirks.trigger_supports_for_each_row:
            stmt += "  FOR EACH ROW\n"
        # ``STATEMENT`` orientation needs no clause for any current
        # dialect (Oracle defaults to statement level when FOR EACH
        # ROW is omitted).

        body_sql = self._format_body(self.definition or "")
        if body_sql:
            stmt += body_sql

        stmt += str(quirks.trigger_terminator)
