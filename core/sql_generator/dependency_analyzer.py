    ...
    def _analyze_object_dependencies(
        ...
        # Tables depend on tables referenced by foreign keys
            ...
            def _ss(key: str, default: Any = None) -> Any:
                ...
                ns = builtin_namespace_for("table_uses_filegroup_syntax")
                if not ns:
                    return default
                return obj.get_dialect_option(ns, key, default=default)

            if _ss("system_versioned", False) and _ss("history_table", None):
                history_schema = _ss("history_schema", None) or getattr(obj, "schema", None)
                history_table = _ss("history_table", None)
                if history_table:
