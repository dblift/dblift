...
class SqlGeneratorFactory:
    ...
    def create(
        ...
        Args:
            dialect: Database dialect name (case-insensitive). ``None`` means
                ...
                nothing when they have no dialect, so ``""`` is left untouched.
        ...
        if dialect is None:
            # No dialect context: render via the registry's reference dialect
            # so the no-dialect default carries no literal in core/.
            from db.provider_registry import ProviderRegistry

            dialect = ProviderRegistry.reference_dialect_name()
