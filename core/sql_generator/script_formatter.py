...
class SqlScriptFormatter:
    ...
    def _format_statement(self, statement: SqlStatement) -> List[str]:
        ...
        # SDK operation hint (e.g. CosmosDB containers)
        if statement.requires_sdk:
            from db.provider_registry import ProviderRegistry

            _quirks = ProviderRegistry.get_quirks((statement.dialect or "").lower())
            hint = _quirks.sdk_operation_hint_prefix()
            if hint:
                lines.append(hint)
                if statement.sdk_operation:
                    op = statement.sdk_operation
                    lines.append(f"-- Operation: {op.get('operation', 'unknown')}")
                    lines.append(f"-- Python: {op.get('python_code', 'N/A')}")
                    if "warning" in op:
        ...
        # SQL statement (or pseudo-SQL for CosmosDB)
