...
class SequenceExtractor(BaseExtractor):
    ...
    def get_sequences(self, schema: str) -> List[Sequence]:
        ...
        ``[]`` means the schema genuinely has no sequences (the vendor
        query ran and returned zero rows) or the dialect has no sequences
        at all. Engines without ``CREATE SEQUENCE`` — SQLite and MySQL
        among them — are declined up front by
        ``vendor_queries.supports_sequences()``, before any query is
        issued, so "this engine has no sequences" never arrives as an
        exception. A failure to *read* sequences — permission denied, a
        ...
        for row in results:
            ...
            if sequence_status:
                sequence_status.add_property_status("start_with", sequence.start_with is not None)
