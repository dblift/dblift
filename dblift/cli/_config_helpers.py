...
_SUBCOMMAND_BOOLEAN_FLAGS = frozenset(
    {
        ...
        "--mark-as-executed",
        "--no-performance",
        "--offline",  # `dblift mcp`: the splitter would otherwise take the next token as a value
...
def _command_handler_attr(command: Optional[str], attr_name: str, default: Any = None) -> Any:
    if not command:
        return default
    from dblift.cli._command_handlers import _COMMAND_HANDLERS
...
def _load_and_merge_config(args: argparse.Namespace, log: Any) -> Any:
    ...
    if db_overrides:
        ...
        if not is_offline:
            from dblift.config.secrets._resolver import resolve_secret_refs
...
def _configure_logging(
    ...
    if getattr(args, "no_progress", False):
        from dblift.core.logger.console import set_progress_disabled
