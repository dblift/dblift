...
def _native_dialect_choices() -> List[str]:
    ...
    """
    # Route through ``api.*`` (not ``dblift.db.provider_registry`` directly): cli/ is
...
def _register_premium_stub_parsers(parser: argparse.ArgumentParser) -> None:
    ...
    """
    from dblift.cli.premium_manifest import premium_commands_missing_from
...
def create_parser(
    ...
    mcp_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Serve only tools that declare themselves read-only; an add-on tool "
            "registered read_only=False is skipped (its declaration is trusted; "
