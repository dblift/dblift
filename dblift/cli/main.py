...
from dblift.cli._command_handlers import (  # noqa: F401
    _AVAILABLE_COMMANDS,
    _COMMAND_HANDLERS,
...
class _CliContext:
    ...
    # ``dblift.core.seams.license_info`` seam: ``None`` in a pure OSS install (no
...
def main() -> None:
    ...
    """
    from dblift.core.logger.console import install_rich_traceback
...
def _parse_argv_and_load_config(argv: List[str]) -> _CliContext:
    ...
    * Extension terminal commands: dispatch and exit with their return code.
    ...
    if commands and commands[0] == "config":
        from dblift.cli.commands.config_command import run_config_command
...
def _build_command_client(ctx: "_CliContext") -> Any:
    ...
    from dblift.core.seams.client_factory import resolve_client_class
...
def _propagate_license_banner(log: Any, license_info: Optional[Any]) -> None:
    ...
    registered on the ``dblift.core.seams.license_info`` seam)."""
    if not license_info:
        return
...
def _setup_logging_and_output(ctx: _CliContext) -> CommandOutput:
    ...
    # Banner routing is centralised in :class:`dblift.cli._output.CommandOutput`
...
def _dispatch_command(ctx: _CliContext, command_output: CommandOutput) -> int:
    ...
    try:
        ...
        from dblift.core.migration.commands import base_command

        if main_header:
            from dblift.core.migration.commands.base_command import _render_main_header_panel
...
if __name__ == "__main__":
    try:
        ...
        from dblift.core.logger.console import get_stderr_console
