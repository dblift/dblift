...
What remains is :mod:`dblift.config._subclasses.dummy_config` — a generic/test config
that is **not** a plugin and therefore has no plugin to register through. It
imports :class:`dblift.config.database_config.BaseDatabaseConfig` and uses
``@register_database_type`` so its class is wired into the
``BaseDatabaseConfig._registry`` at import time. The :mod:`dblift.config.database_config`
