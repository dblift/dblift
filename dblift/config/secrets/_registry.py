...
_AUTOLOAD_FAILED_SCHEMES: Set[str] = set()
...
def register(scheme: str, cls: Type) -> None:
    ...
    _AUTOLOAD_FAILED_SCHEMES.discard(scheme)
...
def register_provider(scheme: str, cls: Type) -> None:
    ...
    """
    from dblift.config.secrets._provider_base import AbstractSecretsProvider
...
def get_provider_class(scheme: str) -> Optional[Type]:
    ...
    if provider is not None:
        return provider
    if scheme in KNOWN_SCHEMES and scheme not in _AUTOLOAD_FAILED_SCHEMES:
        _autoload_bundled_providers()
        provider = _providers.get(scheme)
        if provider is None:
            if any(known_scheme in _providers for known_scheme in KNOWN_SCHEMES):
                _AUTOLOAD_FAILED_SCHEMES.add(scheme)
            else:
                _AUTOLOAD_FAILED_SCHEMES.update(KNOWN_SCHEMES)
    return provider


def _autoload_bundled_providers() -> None:
    """Load installed or source-tree secrets provider registration hooks once."""
    global _AUTOLOAD_ATTEMPTED
    if _AUTOLOAD_ATTEMPTED and KNOWN_SCHEMES.issubset(_providers):
        return
    _AUTOLOAD_ATTEMPTED = True

    for entry_point in entry_points(group="dblift.secrets"):
        register_hook = entry_point.load()
        register_hook()
