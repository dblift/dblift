...
Extracted from ``dblift.config.database_config`` during PR-H10 so the facade
module stays under its 500-line budget. Pure functions with no side effects.

URL masking is delegated to :mod:`dblift.core.utils.url_masking` rather than
...
def mask_url_credentials(url: str) -> str:
    ...
    Single-sources the URL rules from :func:`dblift.core.utils.url_masking.mask_database_url`
