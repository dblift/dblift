...
class SQLiteSchemaOperations:
    ...
    def enumerate_clean_candidates(
        ...
        Returned in clean order: views → triggers → tables.

        OBS-04: clean produces a true empty slate. Internal tables are
        dropped with every other table; the lock manager re-creates the
        lock table on the next ``acquire_migration_lock``.
        """
        ...
        # OBS-04: clean drops internal tables with user tables. Triggers and
        # the table itself are in scope. Table-owned indexes are removed by
        # SQLite when the table is dropped.
        triggers_query = (
            "SELECT name FROM sqlite_master "
    ...
    def clean_schema(self, connection: sqlite3.Connection, schema: str) -> CleanExecutionSummary:
        ...
        This drops all user-created tables, views, and triggers, leaving only
        system tables. SQLite removes table-owned indexes with their tables.
