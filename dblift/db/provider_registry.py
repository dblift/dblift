...
class PluginInfo:
    ...
    # import between ``dblift.db.provider_registry`` and ``dblift.config.database_config``;
...
class NativeDriverManager:
    ...
    def missing_driver_message(db_type: str, plugin_info: PluginInfo) -> str:
        ...
        ``dblift.db.native_connection_manager.describe_missing_driver`` (which
...
class ProviderRegistry:
    ...
    def _load_plugin(cls, plugin_dir: Path) -> Optional[PluginInfo]:
        ...
        try:
            module = importlib.import_module(f"dblift.db.plugins.{plugin_name}")
        except Exception as exc:  # pragma: no cover - defensive
            _logger.warning(f"Failed to import dblift.db.plugins.{plugin_name}: {exc}")
            return None
        ...
        if plugin_py.exists():
            try:
                pm = importlib.import_module(f"dblift.db.plugins.{plugin_name}.plugin")
                ...
                # swallow it.
                _logger.warning(
                    f"Failed to load PLUGIN from dblift.db.plugins.{plugin_name}.plugin: {exc}"
                )
                declared = None
    ...
    def _discover_quirks_class(
        ...
        try:
            module = importlib.import_module(f"dblift.db.plugins.{plugin_name}.quirks")
        except Exception as exc:
    ...
    def reference_dialect_name(cls) -> str:
        ...
        :attr:`dblift.db.base_quirks.BaseQuirks.is_ansi_reference_dialect` True
    ...
    def canonical_dialect_name_for_capability(cls, capability: str) -> Optional[str]:
        ...
        ``capability`` is the name of a boolean :class:`dblift.db.base_quirks.BaseQuirks`
