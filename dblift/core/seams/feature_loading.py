"""Load ``dblift.features`` entry points (add-on feature registration).

Each ``dblift.features`` entry point resolves to a no-arg callable that
performs registrations against the neutral seams
(:mod:`dblift.core.seams.runtime_checks`, :mod:`dblift.core.seams.tier_resolver`, ...).
The open-source package ships none; installed add-on packages register
their runtime checks and resolvers here.
...
empty entry-point result latches immediately unless a distribution listed
in ``_KNOWN_TIER_DISTRIBUTIONS`` is installed but its entry point hasn't
reached this process's metadata view yet, in which case it is retried on a
later call instead of latched.

``DBLIFT_DISABLE_CLI_EXTENSIONS=1`` skips feature loading entirely. This is
an accepted, explicit operator opt-out: with add-on packages installed it
also disables their registered runtime checks and resolvers for API usage
in the same process.
"""
...
# Feature registrations load in a fixed order (``_TIER_LOAD_ORDER`` first,
# unknown third parties in between, name as the tie-breaker) — the same
# ordering contract as cli/extensions.py's command-extension loaders.
_TIER_LOAD_ORDER = {"pro": 0, "enterprise": 2}
_DEFAULT_TIER_LOAD_ORDER = 1

_features_loaded = False

# Must be updated if an add-on distribution is renamed or a new one is
# added -- an unlisted distribution is indistinguishable here from "not
# installed", so _any_tier_distribution_installed silently reverts to the
# pre-fix never-retries behavior for it.
_KNOWN_TIER_DISTRIBUTIONS = ("dblift-pro", "dblift-enterprise")


def _any_tier_distribution_installed() -> bool:
    """Whether a distribution listed in ``_KNOWN_TIER_DISTRIBUTIONS`` is installed.
    ...
    (a plain install, no add-on package at all -- the ordinary case) from
