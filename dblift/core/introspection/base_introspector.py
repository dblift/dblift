...
:class:`dblift.db.base_quirks.BaseQuirks` hooks, so this class itself contains
...
from dblift.core.introspection.result import IntrospectionResult, ObjectCaptureStatus
...
from dblift.db.provider_capabilities import get_provider_driver_display
...
class BaseIntrospector:
    ...
    :mod:`dblift.core.introspection.extractors`, and every dialect-specific
    ...
    def __init__(self, provider: Any, log: Any = None, use_vendor_queries: bool = True) -> None:
        ...
        self.catalog_queries = VendorQueriesFactory.create(self.dialect)
        self.vendor_queries = None
        if use_vendor_queries:
            self.vendor_queries = self.catalog_queries
    ...
    def _get_structural_queries(self) -> Any:
        """Return catalog queries used for structural table/column discovery."""
        return self.vendor_queries or self.catalog_queries
    ...
    def enrich_columns_with_computed(
        ...
        """Delegate to
        :func:`dblift.core.introspection._column_enricher.enrich_columns_with_computed`.
        ...
        """
        from dblift.core.introspection._column_enricher import enrich_columns_with_computed as _impl
    ...
    def enrich_columns_with_identity(
        ...
        """Delegate to
        :func:`dblift.core.introspection._column_enricher.enrich_columns_with_identity`.
        """
        from dblift.core.introspection._column_enricher import enrich_columns_with_identity as _impl
    ...
    def enrich_table_with_partition_scheme(
        ...
        """Delegate to
        :func:`dblift.core.introspection._partition_enricher.enrich_table_with_partition_scheme`.

        Kept as a method so existing callers (and the source-inspection tests
        in ``test_schema_introspector_inline_imports``) continue to work.
        """
        from dblift.core.introspection._partition_enricher import (
    ...
    def get_table_partitions(self, schema: str, table: str) -> List[Any]:
        """Delegate to
        :func:`dblift.core.introspection._partition_enricher.get_table_partitions`.
        """
        from dblift.core.introspection._partition_enricher import get_table_partitions as _impl
    ...
    def introspect_schema(self, schema: str, **kwargs: Any) -> Dict[str, Any]:
        """Delegate to :func:`dblift.core.introspection._schema_orchestrator.introspect_schema`.
        ...
        """
        from dblift.core.introspection._schema_orchestrator import introspect_schema as _impl
