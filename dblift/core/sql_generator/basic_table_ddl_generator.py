...
from dblift.core.sql_generator.base_generator import _schema_prefix_from_object
...
def _builtin_option(table: "Table", capability: str, key: str, default: Any = None) -> Any:
    """Read a built-in ``dialect_options`` value under the namespace owning *capability*.
    ...
    if not namespace:
        return default
    return table.get_dialect_option(namespace, key, default=default)
...
class BasicTableDdlGenerator:
    ...
    def generate_create_statement(self) -> str:
        ...
        try:
            from dblift.core.sql_model.constraint_validator import ConstraintValidator
    ...
    def generate_drop_statement(self) -> str:
        ...
        elif style == "if_exists_cascade_constraints":
            return f"DROP TABLE IF EXISTS {schema_prefix}{table_name} CASCADE CONSTRAINTS"
    ...
    def _format_table_header(self) -> str:
        ...
        table_name = self.table.format_identifier(self.table.name)
        quirks = _quirks_for(self.table.dialect)
        if_not_exists = "IF NOT EXISTS " if quirks.table_create_supports_if_not_exists else ""
        ...
        if self.table.temporary:
            ...
            if style == "global_temporary":
                return f"CREATE GLOBAL TEMPORARY TABLE {if_not_exists}{schema_prefix}{table_name}"
            ...
            return f"CREATE TABLE {if_not_exists}{schema_prefix}{table_name}"
    ...
    def _build_temporal_clause(self, col) -> Optional[str]:
        ...
        quirks = _quirks_for(self.table.dialect)
        cap = "table_uses_filegroup_syntax"
        if not quirks.table_supports_system_versioned or not _builtin_option(
            self.table, cap, "system_versioned", default=False
        ):
            return None
        start_col = str(_builtin_option(self.table, cap, "period_start_column") or "").lower()
        end_col = str(_builtin_option(self.table, cap, "period_end_column") or "").lower()
    ...
    def _build_identity_clause(self, col, dialect: Optional[str] = None) -> Optional[str]:
        ...
        # PostgreSQL is the only dialect whose SERIAL/identity columns encode
        # their generator as a ``nextval(...)`` default — ``serial_types_alias_integer``
        # is set True for PG alone (canonical or ``postgres`` alias), so it
        # captures exactly the membership these branches used to test.
        is_postgresql = quirks.serial_types_alias_integer
    ...
    def _generate_constraint_definitions(
        ...
        ss_cap = "table_uses_filegroup_syntax"
        period_start = _builtin_option(self.table, ss_cap, "period_start_column")
        period_end = _builtin_option(self.table, ss_cap, "period_end_column")
        if (
            quirks.table_supports_system_versioned
            and _builtin_option(self.table, ss_cap, "system_versioned", default=False)
            and period_start
            and period_end
        ):
            period_clause = (
                f"PERIOD FOR SYSTEM_TIME ("
                f"{self.table.format_identifier(period_start)}, "
                f"{self.table.format_identifier(period_end)})"
            )
    ...
    def _generate_table_suffix(self) -> str:
        ...
        ss_cap = "table_uses_filegroup_syntax"
        filegroup = _builtin_option(self.table, ss_cap, "filegroup")
        if filegroup and quirks.table_uses_filegroup_syntax:
            if str(filegroup).upper() == "PRIMARY":
                suffix += " ON [PRIMARY]"
            else:
                suffix += f" ON {self.table.format_identifier(filegroup)}"

        if (
            _builtin_option(self.table, ss_cap, "memory_optimized", default=False)
            and quirks.table_supports_memory_optimized
        ):
            suffix += " WITH (MEMORY_OPTIMIZED = ON)"

        if (
            _builtin_option(self.table, ss_cap, "system_versioned", default=False)
            and quirks.table_supports_system_versioned
        ):
            suffix += " WITH (SYSTEM_VERSIONING = ON"
            history_table_name = _builtin_option(self.table, ss_cap, "history_table")
            history_schema_name = _builtin_option(self.table, ss_cap, "history_schema")
            if history_table_name:
                history_schema = (
                    self.table.format_identifier(history_schema_name)
                    if history_schema_name
                    else (
                        self.table.format_identifier(self.table.schema) if self.table.schema else ""
                    )
                )
                history_table = self.table.format_identifier(history_table_name)
                if history_schema:
        ...
        if quirks.table_supports_storage_params:
            storage_cap = "table_supports_storage_params"
            storage_params = []
            pctfree = _builtin_option(self.table, storage_cap, "pctfree")
            pctused = _builtin_option(self.table, storage_cap, "pctused")
            initial = _builtin_option(self.table, storage_cap, "initial")
            next_extent = _builtin_option(self.table, storage_cap, "next")
            if pctfree is not None:
                storage_params.append(f"PCTFREE {pctfree}")
            if pctused is not None:
                storage_params.append(f"PCTUSED {pctused}")
            if initial is not None:
                storage_params.append(f"INITIAL {initial}")
            if next_extent is not None:
                storage_params.append(f"NEXT {next_extent}")
            if storage_params:
                suffix += f"\n{', '.join(storage_params)}"

        inherits = _builtin_option(self.table, "table_supports_inherits", "inherits", default=[])
        if quirks.table_supports_inherits and inherits:
            formatted_inherits = []
    ...
    def _format_default_value(self, column: SqlColumn) -> Optional[str]:
        ...
        quirks = _quirks_for(self.table.dialect)
        # PostgreSQL is the only dialect that qualifies ``nextval(...)`` export
        # defaults; ``serial_types_alias_integer`` is True for PG alone (canonical
        # or ``postgres`` alias), matching the membership this branch used to test.
        if quirks.serial_types_alias_integer:
