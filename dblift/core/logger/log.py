...
so existing imports of the form
    ...
    from dblift.core.logger.log import LogLevel, FileLog, LogFactory, ...

keep working after the PR-B5 split. The implementations now live in
``_levels.py``, ``_formatters.py``, ``_multi.py``, ``_null.py``,
``_factory.py``. Test patches that target ``dblift.core.logger.log.X`` for
...
from dblift.core.logger._multi import MultiLog  # noqa: F401  re-export
from dblift.core.logger._null import NullLog  # noqa: F401  re-export
from dblift.core.logger.formatters.jsonformatter import JsonFormatter  # noqa: F401  re-export
