...
from ``dblift.core.logger.log`` for back-compat. JSON / HTML formatters live in
...
class TextFormatter(LogFormatter):
    ...
    #: provider via the ``dblift.core.seams.license_info`` seam. ``None`` in a pure
    ...
    def format_header(self, schema: str = None, database_name: str = None) -> str:
        ...
        # ``dblift.core.seams.license_info`` seam). A higher tier that registers a
