...
class MySqlProvider(SqlAlchemyProvider):
    ...
    def list_droppable_objects(self, schema: str) -> List[DroppableObject]:
        """Return MySQL clean candidates in preview order without executing drops."""
        preview = self.get_clean_preview(schema)
        objects = [
            DroppableObject(
                name=cleaned_object.name,
                object_type=cleaned_object.object_type,
                drop_sql=drop_sql,
            )
            for cleaned_object, drop_sql in zip(preview.objects, preview.statements)
        ]
        if objects:
            return [
                DroppableObject(
