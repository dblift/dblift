...
class SQLiteProvider(NativeProvider):
    ...
    def list_droppable_objects(self, schema: str) -> List[DroppableObject]:
        """Return SQLite objects in dependency-safe clean order."""
        connection = self._get_connection()
        candidates = [
            DroppableObject(name=name, object_type=object_type, drop_sql=drop_sql)
            for object_type, name, drop_sql in self.schema_operations.enumerate_clean_candidates(
                connection, schema
            )
        ]
        if candidates:
            return [
                ...
                *candidates,
                DroppableObject(
                    name="foreign_keys_on",
                    object_type="clean_control",
                    drop_sql="PRAGMA foreign_keys = ON",
                    record_result=False,
                ),
            ]
        return candidates
