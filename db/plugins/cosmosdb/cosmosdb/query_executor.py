...
class CosmosDbQueryExecutor(BaseQueryExecutor):
    ...
    def execute_statement(
        ...
        try:
            ...
            # Transient ServiceUnavailable on emulator first-boot is logged at
            # WARNING so retrying callers do not surface a wall of ERROR logs
            # before the operation either succeeds or exhausts retries.
    ...
    def _execute_insert(self, sql: str, params: Optional[List[Any]] = None) -> int:
        ...
        """
        # Substitute positional ? placeholders first. Legacy callers use
        # ? positional params. Must run before @paramN
