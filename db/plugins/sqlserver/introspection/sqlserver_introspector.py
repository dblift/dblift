"""SQL Server :class:`Introspector` — F.3.d plugin-self-containment.

Plugin-located entry point. Inherits all orchestration from
:class:`BaseIntrospector` (post-H.2: dialect-agnostic, routes every
dialect decision through ``SqlserverQuirks``).
...
class SQLServerIntrospector(BaseIntrospector):
    """SQL Server introspector entry point.

    Inherits all orchestration from :class:`BaseIntrospector`. The
    dialect-specific behaviour lives in :class:`SqlserverQuirks` and
    in the per-kind extractors.
    ...
    def get_tables(
        ...
        """Get SQL Server tables through vendor queries for native providers."""
        ...
        for table_name in names:
            ...
            self.enrich_table_with_partition_scheme(schema, table_name, table)
            tables.append(table)
        return tables

    def get_constraints(self, schema: str, table: str) -> List[SqlConstraint]:
        """Get SQL Server PK/FK/UNIQUE/CHECK constraints via vendor queries."""
        if getattr(self.provider, "provider_transport", "native") != "native":
    ...
    def get_indexes(self, schema: str, table: str) -> List[Index]:
        """Get SQL Server indexes via IndexExtractor (vendor queries on native path)."""
        return super().get_indexes(schema, table)


__all__ = ["SQLServerIntrospector"]
