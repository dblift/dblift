"""Oracle :class:`Introspector` — F.3.e plugin-self-containment.
...
class OracleIntrospector(BaseIntrospector):
    """Oracle introspector entry point."""
    ...
    def _is_truthy_catalog_value(value: Any) -> bool:
        """Return True for boolean-ish values projected by Oracle catalog queries."""
        if value is None:
            return False
    ...
    def _constraint_state_kwargs(self, row: Any) -> Dict[str, bool]:
        """Return SqlConstraint state kwargs projected by Oracle catalog queries."""
        return {
            "is_deferrable": self._is_truthy_catalog_value(
                self._get_row_value(row, "is_deferrable")
            ),
            "initially_deferred": self._is_truthy_catalog_value(
                self._get_row_value(row, "initially_deferred")
            ),
            "is_enabled": self._is_truthy_catalog_value(self._get_row_value(row, "is_enabled")),
            "is_validated": self._is_truthy_catalog_value(self._get_row_value(row, "is_validated")),
        }
    ...
    def get_tables(
        ...
        """Get Oracle tables through vendor queries for native providers."""
    ...
    def get_constraints(self, schema: str, table: str) -> List[SqlConstraint]:
        """Get Oracle PK/FK/UNIQUE/CHECK constraints via vendor queries."""
        if getattr(self.provider, "provider_transport", "native") != "native":
        ...
        if pk_result is not None:
            ...
            if pk_rows:
                ...
                pk_state = self._constraint_state_kwargs(pk_rows[0])
                constraints.append(
                    SqlConstraint(
                        ...
                        dialect=self.dialect,
                        is_deferrable=pk_state["is_deferrable"],
                        initially_deferred=pk_state["initially_deferred"],
                        is_enabled=pk_state["is_enabled"],
                        is_validated=pk_state["is_validated"],
                    )
                )
        ...
        if fk_result is not None:
            ...
            for row in fk_rows:
                ...
                if name not in fk_map:
                    fk_map[name] = {
                        ...
                        "state": self._constraint_state_kwargs(row),
                    }
            ...
            for fk in fk_map.values():
                c = SqlConstraint(
                    ...
                    is_deferrable=fk["state"]["is_deferrable"],
                    initially_deferred=fk["state"]["initially_deferred"],
                    is_enabled=fk["state"]["is_enabled"],
                    is_validated=fk["state"]["is_validated"],
                )
        ...
        if uc_sql is not None:
            ...
            uc_map: Dict[str, Any] = {}
            for row in uc_rows:
                name = self._get_row_value(row, "constraint_name") or ""
                if name not in uc_map:
                    uc_map[name] = {
                        "columns": [],
                        "state": self._constraint_state_kwargs(row),
                    }
                uc_map[name]["columns"].append(self._get_row_value(row, "column_name"))
            for name, unique in uc_map.items():
                if unique["columns"]:
                    constraints.append(
                        SqlConstraint(
                            ...
                            dialect=self.dialect,
                            is_deferrable=unique["state"]["is_deferrable"],
                            initially_deferred=unique["state"]["initially_deferred"],
                            is_enabled=unique["state"]["is_enabled"],
                            is_validated=unique["state"]["is_validated"],
                        )
                    )
    ...
    def get_indexes(self, schema: str, table: str) -> List[Index]:
        """Get Oracle indexes via IndexExtractor (vendor queries on native path)."""
        return super().get_indexes(schema, table)


__all__ = ["OracleIntrospector"]
