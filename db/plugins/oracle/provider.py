...
class OracleProvider(SqlAlchemyProvider):
    ...
    def list_droppable_objects(self, schema: str) -> List[DroppableObject]:
        """Return Oracle objects in the same order clean preview reports them."""
        summary = self._clean_schema(schema, execute=False)
        return [
    ...
    def _clean_schema(self, schema: str, execute: bool) -> CleanExecutionSummary:
        ...
        object_queries = [
            ...
            (
                "table",
                "DROP TABLE",
                "SELECT t.TABLE_NAME AS object_name FROM ALL_TABLES t "
