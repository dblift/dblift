...
- **Introspection discarded a column's identity generation kind.**
  `enrich_columns_with_identity` in `core/introspection/_column_enricher.py`
  built a fixed three-key record (`seed_value`, `increment_value`,
  `last_value`) from each identity row, so no identity query could deliver a
  ...
  of every dialect. A column declared `GENERATED ALWAYS AS IDENTITY`
  therefore re-rendered as `GENERATED BY DEFAULT AS IDENTITY` — a migration
  that silently relaxes the constraint, since the column starts accepting
  caller-supplied values it previously rejected, and the emitted DDL is valid
  so neither `diff` nor round-trip validation reports anything.

  An identity query may now return `identity_generation` (`ALWAYS` or
  `BY DEFAULT`) alongside the other three, read with the same
  case-insensitive row access, and the value lands on the column. It stays
  optional: a query that does not select it leaves `identity_generation` at
  `None` rather than a guessed default, which is what dialects whose identity
  syntax has no such concept need. The value is applied on the same path as
  seed and increment, so a column whose identity metadata cannot be decoded
  still falls out of identity handling entirely instead of keeping a
  generation kind. `render_identity_clause` already read the field and is
  unchanged.
