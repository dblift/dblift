...
- Oracle: an unquoted `schema:` value is uppercased everywhere dblift uses it — connecting (`ALTER SESSION SET CURRENT_SCHEMA`), creating the schema/user, the history table and other objects, and every catalog lookup. Oracle uppercases unquoted identifiers, so an unquoted `myschema` is the user `MYSCHEMA`. 4.8.0 used the configured name verbatim everywhere, including connect, so a lowercase `schema:` really did target a lowercase user. A name wrapped in double quotes keeps that exact case through schema readiness, the history table, object creation, and schema cache keys (the key is the catalog spelling, so `myschema` and `"MYSCHEMA"` share a key and `"myschema"` does not collide with them). Quotes are not stripped before the user is created or the session schema is set. If the configured schema is unquoted, is not already uppercase, and `ALL_USERS` contains that exact case-sensitive name, dblift stops before creating any object or history row and tells you to quote the schema name in config to keep the existing lowercase schema. The double quotes are part of the value — in YAML write `schema: '"myschema"'`, because `schema: "myschema"` is just the unquoted name.
...
- Added `dblift.extensions.sql_model`, a stable import path for schema-model
  types and helpers used by third-party plugins.
...
- The `dev` extra now installs the MCP SDK. Without it the tests that speak the
  protocol are skipped and the suite still reports success. It brings the SDK's
  HTTP transport dependencies with it, which `dblift mcp` itself does not use —
  it serves over stdio.
- CI pins the MCP SDK and its dependencies, which were resolved fresh on every
  run instead of from `constraints-ci.txt`.
